...
class LorentzWrappedNormal(torch.nn.Module):
    ...
    def rsample(self, mean_H, covar, num_samples=1, keepdim=False, ret_uv=False):
        ...
        vT = MultivariateNormal(
                        torch.zeros((mean_H.shape[0], mean_H.shape[1]-1), device=covar.device),     # mean_H = [100, 129] 
                        covar                                                                       # covar  = [100, 128, 128]
                    ).rsample((num_samples,))                                                       # vT     = [1, 100, 128]

        #print("\nmultivariate")
        #print(mean_H.shape[0])
        #print(mean_H.shape[1])
        #print(covar.shape)
        #print("end multivariate")
        #print(vT.shape)
    ...
    def log_prob(self, z, mean_H, covar, u=None, v=None):
        ...
        log_det_proj_mu = (nT-1) * (torch.log(torch.sinh(r))-torch.log(r)) # Logarithm rules for easier computation
                                                                           # Eq. (11) in "A wrapped normal distribution on hyperbolic space"
        
        # Compute log likelihood
        logp_z = logp_vT - log_det_proj_mu                                 # Eq. (7) in "A wrapped normal distribution on hyperbolic space"
                                                                           # The idea is the following: According to the sampling algorithm
                                                                           # in H_Embedding, v (vT) is sampled from a multivariate normal
                                                                           # distribution of mean mu0 and covariance covar. The function 
                                                                           # exp_mu(pt_mu0->mu(v)) maps v to z. The relation between log(p(z))
                                                                           # and log(p(v)) is given in Eq. (7). Since log(p(v)) is known, 
                                                                           # we can compute log(p(z)) from this relation. Note that log()
                                                                           # of the second term of the right-hand side of Eq. (8) equal 0
                                                                           # since det(\partial pt_mu0->mu(v)) = 1.  
