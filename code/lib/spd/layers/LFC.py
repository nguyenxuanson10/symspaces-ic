...
class LorentzFullyConnected(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.normalize:
            scale = x.narrow(-1, 0, 1).sigmoid() * self.scale.exp()    # \lambda * \sigma(v^Tx + b') in Eq. (30) of HyperbolicCV paper
            ...
            x_space = scale*unit_length                                # \phi(Wx, v) in Eq. (30) of HyperbolicCV paper
            ...
            x = self.manifold.add_time(x_space)    # Eq. (31) of HyperbolicCV paper: x_space = \psi(Wx + b). This simply adds 
                                                   # sqrt(||\psi(Wx + b)||^2 - 1/K) to the time dimension. For H_encoder, self.normalize  
        return x                                   # is False so this code is always executed and a batch normalization follows. Otherwise,
                                                   # (self.normalize=True), internal normalization will be executed (noted in HypCV paper)
